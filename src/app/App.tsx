...
// Memo 타입 정의 (x, y 퍼센트 대신 lat, lng 위경도 사용)
type Memo = {
  id: number;
  place: string;
  content: string;
...
export default function App() {
  ...
  const saveMemo = () => {
    ...
    // 기본 좌표: 중앙대 정문 좌표 기준
    const memo: Memo = { id: Date.now(), place, content, author: "나", emoji: "📍", shared, radius, time: "방금 저장됨", lat: 37.5051, lng: 126.9571 };
...
function HomeScreen({ memos, onSelect, onCompose }: { memos: Memo[]; selected: Memo | null; onSelect: (m: Memo) => void; onCompose: () => void }) {
  ...
  const places = [
    { name: "이마트 흑석점", category: "대형마트 · 동작구 흑석로 97", lat: 37.5082, lng: 126.9635 },
    { name: "중앙약국", category: "약국 · 동작구 흑석로 102", lat: 37.5071, lng: 126.9585 },
    { name: "중앙대학교 정문", category: "대학교 · 동작구 흑석로 84", lat: 37.5051, lng: 126.9571 },
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative h-full overflow-hidden bg-[#f8f9fa]">
      ...
      <div className="absolute left-4 right-4 top-[52px] z-20">
        ...
        <AnimatePresence>
          {searchOpen && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="mt-2 overflow-hidden rounded-[18px] border border-[#e7eaf1] bg-white shadow-[0_12px_30px_rgba(30,40,70,.14)]">
              <p className="px-4 pb-2 pt-3 text-[11px] font-bold text-primary">장소를 선택하세요</p>
              {places.map((place) => (
                <button key={place.name} onClick={() => handleSelectPlace(place)} className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-[#fafbfe]">
                  <span className="flex size-8 items-center justify-center rounded-xl bg-[#eeecff] text-primary"><MapPin size={15} /></span>
                  <span><b className="block text-[13px]">{place.name}</b><small className="mt-0.5 block text-[10px] text-muted-foreground">{place.category}</small></span>
                </button>
              ))}
            </motion.div>
          )}
